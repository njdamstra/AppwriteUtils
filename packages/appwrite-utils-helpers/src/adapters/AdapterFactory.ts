...
export class AdapterFactory {
  ...
  static async create(config: AdapterFactoryConfig): Promise<AdapterFactoryResult> {
    ...
    // Include auth type in cache key to prevent mixing session and API key adapters
    const authType = config.sessionCookie ? 'session' : (config.appwriteKey ? 'apikey' : 'none');
    const cacheKey = `${config.appwriteEndpoint}:${config.appwriteProject}:${config.apiMode || 'auto'}:${authType}`;
  ...
  static async createFromConfig(config: AppwriteConfig, forceRefresh?: boolean): Promise<AdapterFactoryResult> {
    return this.create({
      ...
      preConfiguredClient: config.appwriteClient
    });
  }
