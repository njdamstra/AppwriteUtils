...
export const getClientFromConfig = (config: AppwriteConfig, sessionCookie?: string) => {
  ...
  if (!config.appwriteClient) {
    // First check if config has a session cookie (from ConfigManager/SessionAuthService)
    const effectiveSession = sessionCookie || config.sessionCookie;
...
export const getClientWithAuth = (
  ...
  // Priority 2: Session from Appwrite CLI prefs (fallback)
...
export const getAdapterFromConfig = async (
  ...
  // First, check if config already has a session (from ConfigManager/SessionAuthService)
  if (!sessionCookie && config.sessionCookie && isValidSessionCookie(config.sessionCookie)) {
    sessionCookie = config.sessionCookie;
  }

  // Fallback: try to find session if still none and no API key
...
export const getAdapter = async (
  ...
  // Priority 1: Explicit session cookie parameter
  if (sessionCookie) {
    if (isValidSessionCookie(sessionCookie)) {
    ...
    // Priority 2: Check if config already has a session (from ConfigManager/SessionAuthService)
    // This is passed through from the config object (e.g., config.sessionCookie)
    const configSessionCookie = (config as any).sessionCookie;
    if (configSessionCookie && isValidSessionCookie(configSessionCookie)) {
      config.appwriteClient = getClientWithAuth(endpoint, project, key, configSessionCookie);
    } else if (!key) {
      // Priority 3: Try to find session auth if no API key provided
      const sessionAuth = findSessionByEndpointAndProject(endpoint, project);
      if (sessionAuth) {
        if (isValidSessionCookie(sessionAuth.sessionCookie)) {
